...
make dev-signed                                       # like `make dev` but codesigns the debug binary with a
                                                     ...
                                                     #   codehub-dev) so macOS Keychain "Always Allow" persists
                                                     #   across rebuilds. Builds --no-default-features (drops
                                                     #   custom-protocol → loads live Vite, not the stale dist).
