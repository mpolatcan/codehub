...
// ─────────────────────────────────────────────────────────────
function DCSection({ id, title, subtitle, children, gap = 48, columns = 5 }) {
  ...
  return (
    <div data-dc-section={sid}
      ...
      <div style={{
        ...
        columnGap: gap,
        // Row gap must clear the floating per-card header (24px) plus breathing
        // room; otherwise the next row's cards crowd the previous row's labels.
        rowGap: 'calc(80px * var(--dc-inv-zoom, 1))',
        padding: '0 60px',
        alignItems: 'start',
        width: 'max-content',
      }}>
...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete }) {
  ...
  // their would-be slots in real time via transforms. In grid layout the
  // slots have both X and Y positions (cards wrap to N rows), so the nearest-
  // slot pick uses Euclidean distance and the displacement is 2D. DOM order
  ...
  const onGripDown = (e) => {
    ...
    const homes = peers.map((el) => {
      ...
      return { el, id: el.dataset.dcSlot, cx: r.left + r.width / 2, cy: r.top + r.height / 2 };
    });
    // Slot positions in order — index N maps to the on-screen home of the Nth
    // child element (which holds order[N] until liveOrder changes).
    const slotPos = homes.map((h) => ({ cx: h.cx, cy: h.cy }));
    ...
    const layout = () => {
      for (const h of homes) {
        ...
        const dx = (slotPos[slot].cx - h.cx) / scale;
        const dy = (slotPos[slot].cy - h.cy) / scale;
    ...
    const move = (ev) => {
      ...
      const cur = { x: homes[startIdx].cx + dx, y: homes[startIdx].cy + dy };
      let nearest = 0, best = Infinity;
      for (let i = 0; i < slotPos.length; i++) {
        const ddx = slotPos[i].cx - cur.x, ddy = slotPos[i].cy - cur.y;
    ...
    const up = () => {
      ...
      const fdx = (slotPos[finalSlot].cx - homes[startIdx].cx) / scale;
      const fdy = (slotPos[finalSlot].cy - homes[startIdx].cy) / scale;
      me.style.transform = `translate(${fdx}px, ${fdy}px)`;
