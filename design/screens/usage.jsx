  ...
  return (
    <AppChrome w={1440} h={900} title="codehub · usage">
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <window.AppSidebar active="usage" />
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-1)', minWidth: 0 }}>
          ...
          <div style={{ padding: '1.25rem 1.75rem 0.875rem', borderBottom: '1px solid var(--bd-soft)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.875rem', marginBottom: '1rem' }}>
              <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600, letterSpacing: '-0.01em' }}>Usage</h1>
              <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-2)' }}>6 accounts · $192.17 / $400 monthly budget · cycle ends in 9 days</span>
              <span style={{ flex: 1 }} />
              <button className="btn sm ghost">Export CSV</button>
              <button className="btn sm">{Ico.plus}Add account</button>
            </div>

            {/* aggregate strip — auto-fit so cards reflow when narrow */}
          ...
          {/* filter row */}
          <div style={{
            padding: '0.625rem 1.75rem', borderBottom: '1px solid var(--bd-soft)',
            ...
            <div className="vr" style={{ height: '1.125rem', margin: '0 0.375rem' }} />
            <button className="btn xs ghost">Subscriptions</button>
            <button className="btn xs ghost">API · pay-as-go</button>
            <span style={{ flex: 1 }} />
            <span className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-3)' }}>updated 4s ago</span>
          </div>

          {/* cards */}
          <div className="scroll" style={{ flex: 1, overflow: 'auto', padding: '1.25rem 1.75rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
  ...
  return (
    <div className="card" style={{ padding: '0.875rem', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
        <span className="mono tnum" style={{ fontSize: '1.375rem', color: toneCol, fontWeight: 500, letterSpacing: '-0.02em' }}>{value}</span>
        {delta && <span className="mono" style={{ fontSize: '0.6875rem', color: dtone }}>{delta}</span>}
      </div>
      {gauge !== undefined && (
        <div style={{ height: 3, background: 'var(--bg-3)', borderRadius: '62.4375rem', overflow: 'hidden' }}>
          <div style={{ width: `${gauge * 100}%`, height: '100%', background: gaugeColor || 'var(--fg-1)' }} />
        </div>
      )}
      <div className="mono" style={{ fontSize: '0.6875rem', color: tone === 'warn' ? 'var(--wait)' : tone === 'over' ? 'var(--err)' : 'var(--fg-2)' }}>{subtle}</div>
    </div>
  );
}
...
function UsageCard({ id, agent, planName, planPrice, renew, status, alert, meters, spark, forecast, upgradeHint }) {
  ...
  return (
    <div className="card" style={{
      ...
      <div style={{ flex: '0 0 280px', padding: '1rem 1.125rem', borderRight: '1px solid var(--bd-soft)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6875rem' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginBottom: 2 }}>
              ...
              <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--fg-0)' }}>{a.name}</span>
            </div>
            <div className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-2)' }}>{planName}</div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: '0.6875rem', color: 'var(--fg-2)', fontFamily: 'var(--mono)' }}>
      ...
      {/* CENTER — meters */}
      <div style={{ flex: 1, padding: '1rem 1.375rem', minWidth: 0, display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
        {alert && (
          <div style={{
            display: 'flex', alignItems: 'center', gap: '0.5rem',
        ...
        <div style={{
          ...
          display: 'flex', alignItems: 'center', gap: '0.5rem',
          fontSize: '0.75rem', color: forecast.tone === 'over' ? 'var(--err)' : forecast.tone === 'warn' ? 'var(--wait)' : 'var(--fg-2)',
        }}>
          <span style={{ fontFamily: 'var(--mono)', fontSize: '0.6875rem', color: 'var(--fg-3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>forecast</span>
          <span>{forecast.text}</span>
        </div>
      </div>

      {/* RIGHT — actions */}
      <div style={{ flex: '0 0 170px', padding: '1rem 1.125rem', borderLeft: '1px solid var(--bd-soft)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {status === 'warn' && <button className="btn sm" style={{ width: '100%', justifyContent: 'center' }}>Raise cap</button>}
        {upgradeHint && <button className="btn sm primary" style={{ width: '100%', justifyContent: 'center', whiteSpace: 'normal', textAlign: 'center', lineHeight: 1.2, padding: '0.5rem 0.625rem' }}>{upgradeHint}</button>}
...
function UsageMeter({ label, used, max, unit, sub }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3125rem' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
        <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-1)' }}>{label}</span>
        <span style={{ flex: 1 }} />
        <span className="mono tnum" style={{ fontSize: '0.8125rem', color: 'var(--fg-0)', fontWeight: 500 }}>
          {fmt(used)}
          <span style={{ color: 'var(--fg-3)', fontWeight: 400 }}> / {fmtMax(max)}</span>
        </span>
      </div>
      <div style={{ height: '0.3125rem', background: 'var(--bg-3)', borderRadius: '62.4375rem', overflow: 'hidden' }}>
        <div style={{ width: `${pct * 100}%`, height: '100%', background: color }} />
      </div>
      {sub && <div className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-3)' }}>{sub}</div>}
    </div>
  );
}
