        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-1)', minWidth: 0 }}>
          ...
          <div style={{ padding: '1.25rem 1.75rem 0.875rem', borderBottom: '1px solid var(--bd-soft)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.875rem' }}>
              <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600, letterSpacing: '-0.01em' }}>Dashboard</h1>
              <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-2)' }}>5 agents · 4 workspaces · $4.42 today</span>
              <span style={{ flex: 1 }} />
              <NewAgentBtn />
            </div>
          </div>

          <div className="scroll" style={{ flex: 1, overflow: 'auto', padding: '1.5rem' }}>
            ...
            {/* SESSIONS TABLE + ATTENTION QUEUE — right column flexes via clamp */}
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) clamp(280px, 22%, 360px)', gap: '0.75rem' }}>
              {/* table */}
              <div className="card" style={{ overflow: 'hidden' }}>
                <div style={{
                  padding: '0.75rem 1rem', borderBottom: '1px solid var(--bd-soft)',
                  display: 'flex', alignItems: 'center', gap: '0.75rem',
                }}>
                  <span style={{ fontSize: '0.8125rem', fontWeight: 500 }}>Sessions</span>
                  ...
                  <span style={{ flex: 1 }} />
                  <span className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-2)' }}>updated 2s ago</span>
                </div>
              ...
              <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '0.75rem 1rem', borderBottom: '1px solid var(--bd-soft)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{ width: '0.375rem', height: '0.375rem', borderRadius: '50%', background: 'var(--wait)' }} />
                  <span style={{ fontSize: '0.8125rem', fontWeight: 500 }}>Needs attention</span>
                  <span style={{ flex: 1 }} />
                  <span className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-2)' }}>2</span>
                </div>
                ...
                <div style={{ padding: '0.75rem', borderTop: '1px solid var(--bd-soft)', marginTop: 'auto' }}>
                  <div className="lbl" style={{ marginBottom: '0.5rem' }}>Workspaces</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginTop: '0.75rem' }}>
              <div className="card" style={{ padding: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.875rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: '0.8125rem', fontWeight: 500 }}>Activity · last 24h</span>
                  <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-2)' }}>turns / hour</span>
                  <span style={{ flex: 1 }} />
              ...
              {/* Account usage card */}
              <div className="card" style={{ padding: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.875rem', marginBottom: '0.75rem' }}>
                  <span style={{ fontSize: '0.8125rem', fontWeight: 500 }}>Accounts · this month</span>
                  <span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-2)' }}>4 active across 3 agents</span>
                  <span style={{ flex: 1 }} />
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: 3 }}>
          <span style={{ fontSize: '0.8125rem', color: 'var(--fg-0)' }}>{a.name}</span>
      ...
      <div style={{ display: 'flex', gap: '0.875rem', fontFamily: 'var(--mono)', fontSize: '0.6875rem', color: 'var(--fg-1)', whiteSpace: 'nowrap' }}>
...
function Metric({ label, value, sub, spark, accent, gauge, delta }) {
  ...
  return (
    <div className="card" style={{ padding: '0.875rem', display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
      <div className="lbl">{label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.625rem' }}>
        <span className="mono tnum" style={{ fontSize: '1.75rem', color: 'var(--fg-0)', fontWeight: 500, letterSpacing: '-0.02em' }}>{value}</span>
        {spark && <Spark data={spark} w={70} h={20} color={accentColor} fill />}
      </div>
      {gauge !== undefined && (
        <div style={{ height: 4, background: 'var(--bg-3)', borderRadius: '62.4375rem', overflow: 'hidden', marginTop: 2 }}>
          <div style={{ width: `${gauge * 100}%`, height: '100%', background: gauge > 0.85 ? 'var(--err)' : gauge > 0.7 ? 'var(--wait)' : 'var(--live)' }} />
        </div>
      )}
      <div className="mono" style={{ fontSize: '0.6875rem', color: accent ? accentColor : deltaColor }}>{sub}</div>
    </div>
  );
}
...
function Th({ children, align }) {
  return (
    <th style={{
      textAlign: align || 'left', padding: '0.5rem 0.875rem',
    ...
    <tr style={{ opacity: dim ? 0.55 : 1, borderBottom: '1px solid var(--bd-soft)' }}>
      ...
      <Td>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
          ...
          {badge && <span style={{ width: '0.4375rem', height: '0.4375rem', borderRadius: '50%', background: 'var(--wait)', boxShadow: '0 0 0 2px color-mix(in oklab, var(--wait) 30%, transparent)' }} />}
        </div>
      </Td>
      <Td><span style={{ color: 'var(--fg-1)' }}>{task}</span></Td>
      <Td><span className="mono" style={{ fontSize: '0.75rem', color: 'var(--fg-2)' }}>{branch}</span></Td>
      ...
      <Td align="right">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.375rem' }}>
          <div className="bar thin" style={{ width: '2.25rem' }}><i style={{ width: `${cpu}%`, background: cpu > 60 ? 'var(--wait)' : 'var(--fg-1)' }} /></div>
          <span className="mono tnum" style={{ minWidth: '1.625rem' }}>{cpu}%</span>
        </div>
      </Td>
...
function Td({ children, align }) {
  return <td style={{ padding: '0.625rem 0.875rem', textAlign: align || 'left', verticalAlign: 'middle' }}>{children}</td>;
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', marginBottom: '0.375rem' }}>
        <AgentGlyph agent={agent} size={13} color={AGENT_META[agent].accent} />
        <span className="mono" style={{ fontSize: '0.75rem' }}>{name}</span>
        <span style={{ flex: 1 }} />
        <span className="mono" style={{ fontSize: '0.6875rem', color: 'var(--fg-3)' }}>{age}</span>
      </div>
      <div style={{ fontSize: '0.8125rem', color: 'var(--fg-0)', marginBottom: 3 }}>{title}</div>
      <div style={{ fontSize: '0.6875rem', color: 'var(--fg-2)', marginBottom: '0.625rem' }}>{sub}</div>
...
function ContainerBar({ name, cpu, mem }) {
  return (
    <div style={{ marginBottom: '0.625rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6875rem', fontFamily: 'var(--mono)', color: 'var(--fg-1)', marginBottom: 4 }}>
        <span>{name}</span>
        <span style={{ color: 'var(--fg-2)' }}>cpu {cpu}% · mem {mem}%</span>
      </div>
      <div style={{ display: 'flex', gap: 3, height: 4 }}>
        <div style={{ flex: 1, background: 'var(--bg-3)', borderRadius: '62.4375rem', overflow: 'hidden' }}>
...
function Legend({ color, label }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3125rem', fontSize: '0.6875rem', color: 'var(--fg-1)' }}>
      <span style={{ width: '0.5625rem', height: 2, background: color, borderRadius: 2 }} />
      {label}
    </span>
  );
}
...
function ActivityChart() {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: '7.5rem' }}>
        {hours.map((d, i) => {
      ...
      <div style={{ display: 'flex', gap: 4, marginTop: '0.375rem', fontFamily: 'var(--mono)', fontSize: '0.625rem', color: 'var(--fg-3)' }}>
        {hours.map((d, i) => (
